...
vec4 resolve_color(
    ...
    return vec4(
        average_color * (1 - transmittance) + (color.rgb*color.a) * transmittance,
        transmittance
    );
}
...
void main(void)
{
    ...
    vec4 opaque = resolve_color(sum_color_tex, sum_weight_tex, opaque_color_tex, frag_uv);

    // do tonemapping
    //opaque = linear_tone_mapping(opaque, 2.0);  // linear color output
    fragment_color.rgb = opaque.rgb; // gamma 2.0 color output
